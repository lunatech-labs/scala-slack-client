...
class PostMessage extends FlatSpec with MockitoSugar {
  ...
  it should "return ResponseMessage when called succesfully" in {
    // Mocking
    ...
    val responseResult = client.postMessage(message)

    //result shouldBe expectedResult

    // Assertions
    ScalaFutures.whenReady(responseResult) { result =>
  ...
  it should "return an error when the call failed" in {
    val slackCaller = mock[SlackCaller]
    val response = mock[StandaloneWSResponse]
    val message = ChatMessage("#general", "Here's a message for you")
    ...
    ScalaFutures.whenReady(responseResult.failed) { failure =>
